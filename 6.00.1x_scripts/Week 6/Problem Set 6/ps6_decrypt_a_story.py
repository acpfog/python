...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        l = len(alphabet)
        for i in range( l ):
            self.cipherDict.update({ alphabet[i] : alphabet[j] })
            self.cipherDict.update({ alphabet[i].upper() : alphabet[j].upper() })
            if j == l - 1:
                j = 0
            else:
                j += 1
        return self.cipherDict
    ...
    def apply_shift(self, shift):
        ...
        for i in range( len(self.message_text) ):
            if self.message_text[i] in keys:
                cipherMessage += cipherDict[self.message_text[i]]
            else:
                cipherMessage += self.message_text[i]
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for shift in range(1,26):
            found = 0
            self.encrypting_dict = self.build_shift_dict( shift )
            for word in self.apply_shift(26-shift).split(' '):
