...
def compChooseWord(hand, wordList, n):
    ...
    for word in wordList:
        ...
        if isValidWord(word, hand, word.split()):
...
def compPlayHand(hand, wordList, n):
    ...
    while calculateHandlen(hand) > 0:
        ...
        if word == None:
            ...
            break
        # Otherwise count score if found
        else:
...
def playGame(wordList):
    ...
    while True:
        ...
        if action == "n" or action == "r":
            if action == "r" and 'hand' not in locals():
                print "You have not played a hand yet. Please play a new hand first!\n"
            else:
                if action == "n":
                    hand = dealHand(HAND_SIZE)
                while True:
                    player = str(raw_input("Enter u to have yourself play, c to have the computer play: ")).lower()
                    if player == "c":
