...
#is the same for all months. Notice that it is possible for the balance to become negative using
...
while True:
    ...
    for month in range ( 1 , paymentPeriod + 1):
        monthlyUnpaidBalance = tmpBalance - minimumFixedMonthlyPayment
        tmpBalance = monthlyUnpaidBalance + ( monthlyUnpaidBalance * monthlyInterestRate )
    if tmpBalance <= 0:
        break

print "Lowest Payment: %s" % minimumFixedMonthlyPayment
