...
lowBond = balance / 12.0
upperBond = ( balance * ( 1 + monthlyInterestRate ) ** 12 ) / 12.0

while True:

    tmpBalance = balance
    monthlyPayment = ( lowBond + upperBond ) / 2

    for month in range ( 1 , 13 ):
        monthlyUnpaidBalance = tmpBalance - monthlyPayment
        tmpBalance = monthlyUnpaidBalance + ( monthlyUnpaidBalance * monthlyInterestRate )

    if tmpBalance > 0:
        lowBond = monthlyPayment
    else:
        upperBond = monthlyPayment

    if tmpBalance > 0 and tmpBalance < 0.001:
        break
